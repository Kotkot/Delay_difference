...
Type objective_function<Type>::operator() ()
{
  ...
  Type sum_S0 = 0;
  for(int i=0;i<n_i;i++){
    S_equil(i) = mu_S_equil(i) * exp(Omega_input(i)/exp(log_tau_O));
    R_equil(i) = mu_R_equil(i) * exp(Omega_input(i)/exp(log_tau_O));
    N_equil(i) = mu_N_equil(i) * exp(Omega_input(i)/exp(log_tau_O));
    W_equil(i) = S_equil(i) / N_equil(i);
    S0(i) = mu_S0(i) * exp(Omega_input(i)/exp(log_tau_O));
